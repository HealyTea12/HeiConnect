...
public:
    ...
    template<typename GraphType, typename LinkGraphType>
    std::tuple<GraphType, LinkGraphType> run(const GraphType& graph, const LinkGraphType& link_graph)
    {
        ...
        using LinkEdgeID = typename LinkGraphType::EdgeID;
        using LinkDistance = typename LinkGraphType::EdgeWeight;
        ...
        for (NodeID u{0}; u < graph.num_vertices(); u++)
        {
            ...
            {
                auto dist = tree_distance(u, v, parent, depth, graph.num_vertices());
        ...
        if constexpr (RecordStatsLevel > 0)
        {
            std::vector<bool> removable_after_project_out = std::vector<bool>(link_graph.num_edges(), false);
            mark_removable_links(link_graph, distance_func, removable_after_project_out);
            size_t num_removed_by_project_out =
                std::count(removable_after_project_out.begin(), removable_after_project_out.end(), true);
            m_metrics = StageMetrics{
                {"num_removed_by_project_in", std::to_string(num_removed_by_project_in)},
                {"num_removed_by_project_out", std::to_string(num_removed_by_project_out)},
                {
                ...
                {
                    "total_removed_links",
                    std::to_string(num_removed_by_project_out),
                },
                {
                    "removed_links_after_project_in",
                    std::to_string(num_removed_by_project_in),
                },
                {
                    "removed_links_after_project_out",
                    std::to_string(num_removed_by_project_out - num_removed_by_project_in),
                }};
        }
        ...
        if constexpr (RecordStatsLevel > 0)
        {
            ...
            m_metrics = StageMetrics{
                {"num_removed_links", std::to_string(num_removed)},
                {
                    "cactus_block_tree_construction_time",
    ...
    private:
        template<typename LinkGraphType>
        void mark_removable_links(
            const LinkGraphType& distance_oracle.get_distance link_graph,
            std::function<
                typename LinkGraphType::EdgeWeight(typename LinkGraphType::NodeID, typename LinkGraphType::NodeID)>
                distance_func,
            std::vector<bool>& removable)
        {
            using NodeID = typename LinkGraphType::NodeID;
