    ...
    const std::vector<link_weight_T> &link_weights)
{
    ...
    std::vector<bool> visited(bt_n, false);
    ...
    for (node_T u{0}; u < link_vertices.size() - 1; u++)
    {
        for (edge_T e{link_vertices[u]}; e < link_vertices[u + 1]; e++)
        {
            ...
            bool found = false;
            stack = std::stack<node_T>();
            visited.assign(bt_n, false);
            parent_bt.assign(bt_n, bt_n);
            parent_bt[v] = v;
            stack.push(v);
            while (!stack.empty())
            {
                ...
                for (edge_T e{block_tree.graph.vertices[current_node]}; e < block_tree.graph.vertices[current_node + 1]; e++)
                {
                    ...
                    if (neighbor == parent_bt[current_node] || visited[neighbor])
                        continue;
            ...
            // backtrack from u to v and mark cycle crosses
            node_T current_node = u;
            while (current_node != v)
            {
                node_T p = parent_bt[current_node];
                if (p > original_graph.num_vertices() - 1) // cycle node
                {
                    cycle_id_T cid = static_cast<cycle_id_T>(p - original_graph.num_vertices());

                    auto a = cycle_positions[cid][current_node];
                    auto b = cycle_positions[cid][parent_bt[p]];
                    if (a > b)
                    {
                        std::swap(a, b);
                    }
