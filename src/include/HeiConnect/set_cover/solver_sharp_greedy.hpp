...
concept GreedySharpContextCon = requires(Context context, size_t set_index, const SetCoverType& set_cover) {
...
public:
    ...
    template<typename SetCoverType, typename Context>
    Output solve(const SetCoverType& set_cover, const Context& context)
    {
        Output output{};
               ...
               context.get_solution_size() != set_cover.get_num_sets())
        {
            for (size_t e = 0; e < set_cover.get_num_elements(); e++)
            {
                ...
                set_cover.forEachSetCoveringElement(
                    e,
                    [&set_cover, &cheapest_set_cost, &cheapest_set](size_t set_index) {
